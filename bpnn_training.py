...
from DToolbox.DBPNNs import DBPNN_moon
...
def bpnn_train_moon(model_path=None, b_save=False):
    ...
    moon = DBPNN_moon(input_dim, input_length, output_dim, output_length, lr)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        weights = moon.weights.eval()
        biases = moon.biases.eval()

        with open('bpnn_wb.pickle', 'wb') as fw:
            pickle.dump(weights, fw)
            pickle.dump(biases, fw)
...
if __name__ == '__main__':
    model_path = 'bpnn_network/bpnn.pickle'
    bpnn_train_moon(model_path, b_save=True)
